...
Which would give a p-value of p = `r pf(f_obs, 2, 6, lower.tail = FALSE) %>% round(3)` and again could be written up as F(2, 6) = `r f_obs`, p = `r pf(f_obs, 2, 6, lower.tail = FALSE) %>% round(3)`, suggesting no significant effect of ambiance or location on concentration as measured through the number of typos.
...
You didn't need to store the output but we have done it out of habit as often we want to do something with the results.  And remember we said that you get two tables in this output.  One of the tables is Levene's Test for Homogeneity of Variance and looks like:
